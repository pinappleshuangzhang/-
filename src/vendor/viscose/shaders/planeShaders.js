...
export const fragmentShader = /* glsl */ `
  ...
  uniform float uHoverPlane; // hovered plane index, -1 when absent
  uniform float uHoverColor; // 0 grayscale, 1 authored colour
  ...
  void main() {
    ...
    for (int i = 0; i < MAX_PLANES; i++) {
      ...
      if (di < d0) {
        d1 = d0; uv1 = uv0; im1 = im0; dm1 = dm0; pi1 = pi0;
        d0 = di; uv0 = luv; im0 = st.w; dm0 = st.z; pi0 = float(i);
      } else if (di < d1) {
        d1 = di; uv1 = luv; im1 = st.w; dm1 = st.z; pi1 = float(i);
      }
    }
    ...
    if (uTextured > 0.5) {
      ...
      float gray0 = dot(c0, vec3(0.2126, 0.7152, 0.0722));
      float gray1 = dot(c1, vec3(0.2126, 0.7152, 0.0722));
      float color0 =
        (1.0 - step(0.5, abs(pi0 - uHoverPlane))) * uHoverColor;
      float color1 =
        (1.0 - step(0.5, abs(pi1 - uHoverPlane))) * uHoverColor;
      c0 = mix(vec3(gray0), c0, color0);
      c1 = mix(vec3(gray1), c1, color1);
